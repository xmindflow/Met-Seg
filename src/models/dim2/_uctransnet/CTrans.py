...
class Reconstruct(nn.Module):
    ...
    def forward(self, x):
        ...
        (
            ...
            n_patch,
            hidden,
        ) = x.size()  # reshape from (B, n_patch, hidden) to (B, h, w, hidden)
...
class Attention_org(nn.Module):
    ...
    def forward(self, emb1, emb2, emb3, emb4, emb_all):
        ...
        multi_head_Q1 = (
            torch.stack(multi_head_Q1_list, dim=1) if emb1 is not None else None
        )
        multi_head_Q2 = (
            torch.stack(multi_head_Q2_list, dim=1) if emb2 is not None else None
        )
        multi_head_Q3 = (
            torch.stack(multi_head_Q3_list, dim=1) if emb3 is not None else None
        )
        multi_head_Q4 = (
            torch.stack(multi_head_Q4_list, dim=1) if emb4 is not None else None
        )
        ...
        attention_scores1 = (
            torch.matmul(multi_head_Q1, multi_head_K) if emb1 is not None else None
        )
        attention_scores2 = (
            torch.matmul(multi_head_Q2, multi_head_K) if emb2 is not None else None
        )
        attention_scores3 = (
            torch.matmul(multi_head_Q3, multi_head_K) if emb3 is not None else None
        )
        attention_scores4 = (
            torch.matmul(multi_head_Q4, multi_head_K) if emb4 is not None else None
        )

        attention_scores1 = (
            attention_scores1 / math.sqrt(self.KV_size) if emb1 is not None else None
        )
        attention_scores2 = (
            attention_scores2 / math.sqrt(self.KV_size) if emb2 is not None else None
        )
        attention_scores3 = (
            attention_scores3 / math.sqrt(self.KV_size) if emb3 is not None else None
        )
        attention_scores4 = (
            attention_scores4 / math.sqrt(self.KV_size) if emb4 is not None else None
        )

        attention_probs1 = (
            self.softmax(self.psi(attention_scores1)) if emb1 is not None else None
        )
        attention_probs2 = (
            self.softmax(self.psi(attention_scores2)) if emb2 is not None else None
        )
        attention_probs3 = (
            self.softmax(self.psi(attention_scores3)) if emb3 is not None else None
        )
        attention_probs4 = (
            self.softmax(self.psi(attention_scores4)) if emb4 is not None else None
        )
        ...
        attention_probs1 = (
            self.attn_dropout(attention_probs1) if emb1 is not None else None
        )
        attention_probs2 = (
            self.attn_dropout(attention_probs2) if emb2 is not None else None
        )
        attention_probs3 = (
            self.attn_dropout(attention_probs3) if emb3 is not None else None
        )
        attention_probs4 = (
            self.attn_dropout(attention_probs4) if emb4 is not None else None
        )

        multi_head_V = multi_head_V.transpose(-1, -2)
        context_layer1 = (
            torch.matmul(attention_probs1, multi_head_V) if emb1 is not None else None
        )
        context_layer2 = (
            torch.matmul(attention_probs2, multi_head_V) if emb2 is not None else None
        )
        context_layer3 = (
            torch.matmul(attention_probs3, multi_head_V) if emb3 is not None else None
        )
        context_layer4 = (
            torch.matmul(attention_probs4, multi_head_V) if emb4 is not None else None
        )

        context_layer1 = (
            context_layer1.permute(0, 3, 2, 1).contiguous()
            if emb1 is not None
            else None
        )
        context_layer2 = (
            context_layer2.permute(0, 3, 2, 1).contiguous()
            if emb2 is not None
            else None
        )
        context_layer3 = (
            context_layer3.permute(0, 3, 2, 1).contiguous()
            if emb3 is not None
            else None
        )
        context_layer4 = (
...
class ChannelTransformer(nn.Module):
    def __init__(
        ...
        self.embeddings_1 = Channel_Embeddings(
            config, self.patchSize_1, img_size=img_size, in_channels=channel_num[0]
        )
        self.embeddings_2 = Channel_Embeddings(
            config, self.patchSize_2, img_size=img_size // 2, in_channels=channel_num[1]
        )
        self.embeddings_3 = Channel_Embeddings(
            config, self.patchSize_3, img_size=img_size // 4, in_channels=channel_num[2]
        )
        self.embeddings_4 = Channel_Embeddings(
            config, self.patchSize_4, img_size=img_size // 8, in_channels=channel_num[3]
        )
